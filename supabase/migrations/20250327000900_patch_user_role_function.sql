-- Patch: Clean and fix update_user_role_with_validation()
-- Fixes ambiguous user_id, reserved keyword usage, and adds clarity

DROP FUNCTION IF EXISTS update_user_role_with_validation(UUID, TEXT);

CREATE FUNCTION update_user_role_with_validation(target_user_id UUID, new_role TEXT)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
