...
-- Consolidated function for safely updating user roles
...
BEGIN
  ...
  IF current_user_role != 'admin'::user_role THEN
    RETURN jsonb_build_object('success', false, 'error', 'Only admins can change user roles', 'status', 403);
  END IF;
  ...
  IF NOT valid_role THEN
    RETURN jsonb_build_object('success', false, 'error', 'Invalid role', 'status', 400);
  END IF;
