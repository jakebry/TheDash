...
-- First check if any business has NULL created_by and fix it
UPDATE businesses
SET created_by = (
  SELECT user_id 
...
BEGIN
  ...
  LOOP
    -- Ensure business creator has an owner role
...
DECLARE
  ...
  roles_fixed INT := 0;
BEGIN
  -- For each business member without a business_user_roles record
  ...
  LOOP
    -- If this user is the business creator, make them an owner
    IF EXISTS (
      SELECT 1
      FROM businesses b
    ...
    ELSE
      -- Otherwise make them a regular employee
      INSERT INTO business_user_roles (business_id, user_id, role)
  ...
  -- Fix any owner mismatches - ensure business creators are owners
...
-- Run the function to fix any existing data issues
SELECT fix_missing_business_roles();

-- Function to get all roles for a user in a business
...
-- Function to get a user's role in a specific business
...
-- Improved update_business_role function
...
BEGIN
  ...
  -- Don't allow changing a business creator from "owner" role
  IF EXISTS (
    SELECT 1
    FROM businesses
